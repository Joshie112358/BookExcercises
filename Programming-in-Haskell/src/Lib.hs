...
takeWhileh f [] = []
takeWhileh f (x:xs) = if f x then x : takeWhileh f xs else []


dropWhileh :: (a -> Bool) -> [a] -> [a]
dropWhileh f [] = []
dropWhileh f (x:xs) = if f x then dropWhileh f xs else (x:xs)
...
-}
luhn2 :: [Int] -> Bool
luhn2 x |sum (altMap luhnDouble id x) `mod` 10 == 0 = True
        |otherwise = False
